...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    while (true)
    {
        ...
        if (xQueueReceive(xQueueFrameI, &frame, portMAX_DELAY))
        {
			...
			//ESP_LOGE("httpd", "get frame in while %d\n",keycnt++);
...
static esp_err_t status_handler_test(httpd_req_t *req)
{
	static int keycnt=0;
	...
	snprintf(json_response,sizeof(json_response),\
			" hello world,httpd send msg %d to you!  .",keycnt++);
...
static esp_err_t btn_handler(httpd_req_t *req)
{
    char *buf = NULL;
	char btnName[32] = {0};	
	static char cmdbuf[64] = {0}; //要发送给stm32,一直留着 ,切不可释放

	ESP_LOGI(TAG, "httpd btn_handler");


    if (parse_get(req, &buf) != ESP_OK ||
        httpd_query_key_value(buf, "button", btnName, sizeof(btnName)) != ESP_OK )
    {
        free(buf);
        httpd_resp_send_404(req);
        return ESP_FAIL;
    ...
    ESP_LOGI(TAG, "httpd detect Btn %s",btnName);
	snprintf(cmdbuf,sizeof(cmdbuf),"toSTM32:action=move,arg=%s;",btnName+3);
	uart_cmd_push2Stm32( (long)  cmdbuf);

    httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
    return httpd_resp_send(req, "got btn", strlen("got btn"));
}
...
void register_httpd(const QueueHandle_t frame_i, const QueueHandle_t frame_o, const bool return_fb)
{
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
		...
		httpd_register_uri_handler(camera_httpd, &btn_uri);
