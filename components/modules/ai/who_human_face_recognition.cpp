...
static void task_process_handler(void *arg)
{
	...
	printf("%s-%d wqf recognize is 8bit model\n",__func__,__LINE__);
#elif CONFIG_S16
    FaceRecognition112V1S16 *recognizer = new FaceRecognition112V1S16();
	printf("%s-%d wqf recognize is 16bit model\n",__func__,__LINE__);
            ...
            if (xQueueReceive(xQueueFrameI, &frame, portMAX_DELAY))
            {
                ...
                if (is_detected)
                {
                    ...
                    if (recognize_result.id > 0)
                        ESP_LOGI("RECOGNIZE success", "Similarity: %f, Match ID: %d", recognize_result.similarity, recognize_result.id);
                    else
                        ESP_LOGE("RECOGNIZE failed", "Similarity: %f, Match ID: %d", recognize_result.similarity, recognize_result.id);
                    frame_show_state = SHOW_STATE_RECOGNIZE; 	//发现人脸,执行recognize操作
                    ...
                    default:
						printf("%s-%d default _gEvent = %d\n",__func__,__LINE__,_gEvent);
                        break;
                    }
                }
                ...
                if (frame_show_state != SHOW_STATE_IDLE)
                {
                    ...
                    case SHOW_STATE_DELETE:
                        rgb_printf(frame, RGB565_MASK_RED, "DELETE: %d IDs left", recognizer->get_enrolled_id_num());
                        break;

                    case SHOW_STATE_RECOGNIZE:
                        if (recognize_result.id > 0)
                            rgb_printf(frame, RGB565_MASK_GREEN, "RECOGNIZE: ID %d", recognize_result.id);
                        else
                            rgb_print(frame, RGB565_MASK_RED, "RECOGNIZE :who ?");
                        break;
                    ...
                    default:
						printf("%s-%d default frame_show_state = %d\n",__func__,__LINE__,frame_show_state);
						
                        break;
                    }

                    if (++frame_count > FRAME_DELAY_NUM)		//rgb_printf画面打停留的时间
                    {
                        frame_count = 0;
